...
-(void)loadAdDataThenStart:(int) prefix{
    ...
    for (int i=0; i<self.adDataArray.count; i++) {
        // 按钮TAG值
        NSString *key = [NSString stringWithFormat:@"%d%d",prefix,i];
        
        NSString *adImg=self.adDataArray[i];
        ...
        [adBtn sd_setImageWithURL:[NSURL URLWithString: adImg] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"banner"]];
        [adBtn sd_setImageWithURL:[NSURL URLWithString: adImg] forState:UIControlStateHighlighted placeholderImage:[UIImage imageNamed:@"banner"]];
        [adBtn setContentMode:UIViewContentModeScaleToFill];
        adBtn.tag= [key intValue];
    ...
    NSString *lastAdImg=self.adDataArray[self.adDataArray.count-1];
    UIButton *lastAdBtn=[[UIButton alloc]initWithFrame:CGRectMake(0, 0, CGRectW(self.adScrollView), CGRectH(self.adScrollView))];
    [lastAdBtn sd_setImageWithURL:[NSURL URLWithString: lastAdImg] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"banner"]];
    [lastAdBtn sd_setImageWithURL:[NSURL URLWithString: lastAdImg] forState:UIControlStateHighlighted placeholderImage:[UIImage imageNamed:@"banner"]];
    [lastAdBtn setContentMode:UIViewContentModeScaleToFill];
    [self.adScrollView addSubview:lastAdBtn];
    
    NSString *firstImg=self.adDataArray[0];
    ...
    [firstAdBtn sd_setImageWithURL:[NSURL URLWithString: firstImg] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"banner"]];
    [firstAdBtn sd_setImageWithURL:[NSURL URLWithString: firstImg] forState:UIControlStateHighlighted placeholderImage:[UIImage imageNamed:@"banner"]];
